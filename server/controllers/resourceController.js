import * as crudService from "../services/crudService.js";
import { isValidTable } from "../db.js";

export function createResourceController(table, options = {}) {
  if (!isValidTable(table)) {
    throw new Error(`Invalid table name: ${table}`);
  }

  const { onDelete } = options;

  async function list(req, res) {
    try {
      const userId = req.user.id;
      const records = await crudService.getAll(table, userId);
      return res.json({ success: true, data: records });
    } catch (error) {
      const statusCode = error.statusCode || 500;
      return res.status(statusCode).json({
        success: false,
        message: error.message || `Failed to fetch ${table}`,
      });
    }
  }

  async function get(req, res) {
    try {
      const userId = req.user.id;
      const record = await crudService.getById(table, req.params.id, userId);
      if (!record) {
        return res.status(404).json({ success: false, message: `${table} not found` });
      }
      return res.json({ success: true, data: record });
    } catch (error) {
      const statusCode = error.statusCode || 500;
      return res.status(statusCode).json({
        success: false,
        message: error.message || `Failed to fetch ${table}`,
      });
    }
  }

  async function createHandler(req, res) {
    try {
      const userId = req.user.id;
      await crudService.create(table, req.body, userId);
      const all = await crudService.getAll(table, userId);
      return res.status(201).json({ success: true, data: all });
    } catch (error) {
      const statusCode = error.statusCode || 400;
      return res.status(statusCode).json({
        success: false,
        message: error.message || `Failed to create ${table}`,
      });
    }
  }

  async function updateHandler(req, res) {
    try {
      const userId = req.user.id;
      await crudService.update(table, req.params.id, req.body, userId);
      const all = await crudService.getAll(table, userId);
      return res.json({ success: true, data: all });
    } catch (error) {
      const statusCode = error.statusCode || 400;
      return res.status(statusCode).json({
        success: false,
        message: error.message || `Failed to update ${table}`,
      });
    }
  }

  async function removeHandler(req, res) {
    try {
      const userId = req.user.id;
      await crudService.remove(table, req.params.id, userId);
      if (onDelete) {
        await onDelete(req.params.id, userId);
      }
      const all = await crudService.getAll(table, userId);
      return res.json({ success: true, data: all });
    } catch (error) {
      const statusCode = error.statusCode || 400;
      return res.status(statusCode).json({
        success: false,
        message: error.message || `Failed to delete ${table}`,
      });
    }
  }

  return { list, get, create: createHandler, update: updateHandler, remove: removeHandler };
}
