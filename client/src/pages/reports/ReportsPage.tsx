...
type ReportType = "summary" | "teacher" | "class" | "utilization" | "quality";
...
export function ReportsPage() {
  ...
  const totalPlacedSlots = useMemo(() => {
    let count = 0;
    Object.values(generatedTimetable).forEach((dayData) => {
  ...
  const conflictFreeRate = useMemo(() => {
    if (allocations.length === 0) return 100;
    const cleanCount = Math.max(0, allocations.length - conflicts.length);
    return Math.round((cleanCount / allocations.length) * 100);
  }, [allocations.length, conflicts.length]);

  const placementCoverage = useMemo(() => {
    if (totalAllocated === 0) return 0;
    return Math.min(100, Math.round((totalPlacedSlots / totalAllocated) * 100));
  }, [totalAllocated, totalPlacedSlots]);
  ...
  const generateCSV = () => {
    ...
    switch (reportType) {
      ...
      case "quality":
        csv = "Metric,Value\n";
        csv += `Health Score,${healthScore}%\n`;
        csv += `Conflict Free Rate,${conflictFreeRate}%\n`;
        csv += `Placement Coverage,${placementCoverage}%\n`;
        csv += `Total Placed Slots,${totalPlacedSlots}\n`;
        csv += `Unplaced Allocations,${unplacedAllocations?.length || 0}\n`;
        csv += `Critical Conflicts,${criticalCount}\n`;
        csv += `Warning Conflicts,${warningCount}\n`;
        break;
      default:
        csv = "Metric,Value\n";
  ...
  const generateJSON = () => {
    ...
    switch (reportType) {
      ...
      case "quality":
        data = {
          report: "Timetable Quality & Optimizer Performance",
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {[
          ...
          { key: "quality" as ReportType, label: "Quality & Optimizer", icon: Activity },
        ].map((tab) => {
          const Icon = tab.icon;
          return (
            <button
        ...
        {reportType === "quality" && (
          <div>
            <h2 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
              ...
              Timetable Quality & Optimizer Performance
            </h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                <div className="flex items-center gap-2 text-slate-500 text-xs font-bold uppercase mb-1">
                  <ShieldCheck className="w-4 h-4 text-emerald-600" /> Timetable Health
                </div>
                <div className="text-3xl font-extrabold text-slate-900">{healthScore}%</div>
                <div className="mt-2 text-xs text-slate-500">Overall constraint compliance</div>
              </div>

              <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                <div className="flex items-center gap-2 text-slate-500 text-xs font-bold uppercase mb-1">
                  <Percent className="w-4 h-4 text-blue-600" /> Conflict-Free Rate
                </div>
                <div className="text-3xl font-extrabold text-slate-900">{conflictFreeRate}%</div>
                <div className="mt-2 text-xs text-slate-500">Allocations without issues</div>
              </div>

              <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                <div className="flex items-center gap-2 text-slate-500 text-xs font-bold uppercase mb-1">
                  <Zap className="w-4 h-4 text-indigo-600" /> Placement Coverage
                </div>
                <div className="text-3xl font-extrabold text-slate-900">{placementCoverage}%</div>
                <div className="mt-2 text-xs text-slate-500">{totalPlacedSlots} / {totalAllocated} slots placed</div>
              </div>

              <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                <div className="flex items-center gap-2 text-slate-500 text-xs font-bold uppercase mb-1">
                  <AlertTriangle className="w-4 h-4 text-amber-600" /> Active Conflicts
                </div>
                <div className="text-3xl font-extrabold text-slate-900">{conflicts.length}</div>
                <div className="mt-2 text-xs text-slate-500">{criticalCount} critical, {warningCount} warning</div>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="rounded-xl border border-slate-200 p-4">
                <h3 className="text-sm font-bold text-slate-700 mb-3">Optimizer Solver Stats</h3>
                <div className="space-y-2.5 text-sm">
                  <div className="flex justify-between"><span className="text-slate-500">Generation Status</span><span className="font-bold text-emerald-600">{timetableExists ? "Active Schedule" : "Ready"}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">Placed Slots</span><span className="font-semibold text-slate-800">{totalPlacedSlots}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">Unplaced Allocations</span><span className="font-semibold text-slate-800">{unplacedAllocations?.length || 0}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">Solver Engine</span><span className="font-semibold text-slate-800">Backtracking + Heuristic</span></div>
                </div>
              </div>

              <div className="rounded-xl border border-slate-200 p-4">
                <h3 className="text-sm font-bold text-slate-700 mb-3">Quality Breakdown</h3>
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-slate-600">Conflict-Free Allocations</span>
                      <span>{conflictFreeRate}%</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 rounded-full" style={{ width: `${conflictFreeRate}%` }}></div>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-xs font-semibold mb-1">
                      <span className="text-slate-600">Slot Placement Coverage</span>
                      <span>{placementCoverage}%</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${placementCoverage}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            Export {reportType === "summary" ? "Summary" : reportType === "teacher" ? "Workload" : reportType === "class" ? "Class" : reportType === "utilization" ? "Subject" : "Quality"} Report
          </button>
        </div>
      </div>
    </div>
